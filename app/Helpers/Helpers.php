...
class Helpers
{
  ...
  public static function setRTL(): bool
  {
    $cacheKey = self::generateCacheKey();

    if (!self::isRTL()) {
      // Cache the RTL mode for 1 week
      cache()->put($cacheKey, true, now()->addWeek());
      return true; // Successfully set to RTL
    }

    return false; // Already in RTL mode
  }

  /**
   * Set the layout to LTR mode.
   *
   * @return bool True if the mode was set to LTR, False if already in LTR.
   */
  public static function setLTR(): bool
  {
    $cacheKey = self::generateCacheKey();

    if (self::isRTL()) {
      // Remove the RTL cache
      cache()->forget($cacheKey);
      cache()->put($cacheKey, false, now()->addWeek());
      return true; // Successfully set to LTR
    }

    return false; // Already in LTR mode
  }

  /**
   * Determine if RTL mode is currently active.
   *
   * @return bool True if RTL mode is active, False otherwise.
   */
  ...
  /**
   * Generate a unique cache key for layout mode based on user context.
   *
   * @return string
   */
  protected static function generateCacheKey(): string
  {
    $userId = auth()->id(); // Use user ID if logged in
    $ip = request()->ip();
    $userAgent = request()->header('User-Agent');

    // If user is authenticated, prioritize user ID, otherwise use IP and User-Agent
    return 'layoutMode_' . md5(($userId ?? $ip) . $userAgent);
  }
